...
metrics.registerGauge('health_check_status', 'Health check status (1=healthy, 0=unhealthy)');
...
export function recordRequest(method: string, path: string, status: number, durationMs: number): void {
  ...
  if (status >= 500) {
    metrics.incrementCounter('http_requests_errors', { method, path, status: String(status) });
  }
