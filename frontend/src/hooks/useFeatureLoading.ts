...
export function useMultiFeatureLoading(features: FeatureType[]) {
  ...
  const isAnyMutationLoading = loadingStates.some(state => state.isMutationLoading);
  const isAnyLoading = loadingStates.some(state => state.isLoading);
  
  const totalActiveQueries = loadingStates.reduce((sum, state) => sum + state.activeQueries, 0);
  const totalActiveMutations = loadingStates.reduce((sum, state) => sum + state.activeMutations, 0);
  ...
  return {
     ...
     */
    featureStates: features.reduce((acc, feature, index) => {
      acc[feature] = loadingStates[index];
      return acc;
