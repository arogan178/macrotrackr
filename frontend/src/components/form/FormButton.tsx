...
function FormButton({
  ...
  const variantStyles = {
    [BUTTON_VARIANTS.PRIMARY]: "bg-indigo-600 hover:bg-indigo-700 text-white focus:ring-indigo-500 shadow-sm",
    [BUTTON_VARIANTS.SECONDARY]: "bg-gray-700 hover:bg-gray-600 text-gray-100 border border-gray-600 focus:ring-gray-500",
    [BUTTON_VARIANTS.DANGER]: "bg-red-600 hover:bg-red-700 text-white focus:ring-red-500 shadow-sm",
    [BUTTON_VARIANTS.SUCCESS]: "bg-green-600 hover:bg-green-700 text-white focus:ring-green-500 shadow-sm",
    [BUTTON_VARIANTS.GHOST]: "bg-transparent hover:bg-gray-700/50 text-gray-300 hover:text-white",
  };
  ...
  const renderContent = () => {
    ...
    return (
      <span className="flex items-center justify-center">
        {icon && iconPosition === ICON_POSITIONS.LEFT && <span className="mr-2">{icon}</span>}
        {text && <span>{text}</span>}
        {icon && iconPosition === ICON_POSITIONS.RIGHT && <span className="ml-2">{icon}</span>}
      </span>
    );
  };
