...
import { InfoCard } from "./FormComponents";
...
export default function MacroDistribution({
  ...
  const handleChange = (macro: "protein" | "carbs" | "fats", value: number) => {
    // ... keeping all the handleChange logic unchanged ...
    ...
    if (lockedMacros.length === 0) {
      ...
      // ... rest of the locked macros logic ...
      ...
      if (unlockedMacros.length === 1) {
        ...
        updatedDistribution[unlockedMacroKey] = Math.round(
          ...
          if (otherLockedMacros.length > 0) {
            const highestLockedMacro = otherLockedMacros.reduce(
              (highest, current) => {
                ...
                return updatedDistribution[currentKey] >
                  updatedDistribution[highestKey]
                  ? current
                  : highest;
              },
              otherLockedMacros[0]
            ...
            if (newHighestValue >= 5 && newHighestValue <= 70) {
              updatedDistribution[highestLockedKey] =
    ...
    Object.keys(lockedValues).forEach((key) => {
      if (key !== macroKey || !isCurrentMacroLocked) {
        updatedDistribution[key as keyof typeof updatedDistribution] =
          lockedValues[key];
      }
    });
