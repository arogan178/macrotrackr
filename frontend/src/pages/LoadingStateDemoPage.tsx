    ...
    <div className="min-h-screen bg-gray-900 text-white">
      ...
      <div className="max-w-6xl mx-auto p-6 space-y-8">
        <header className="text-center">
          <h1 className="text-3xl font-bold mb-2">Loading State Components Demo</h1>
          <p className="text-gray-400">Interactive demonstration of all loading state patterns</p>
        </header>
...
// Loading State Information Panel
function LoadingStateInfoPanel({ globalLoading, criticalLoading, habitsLoading }: any) {
  return (
    <div className="bg-gray-800 rounded-lg p-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
        ...
        <div className="space-y-2">
          ...
          <div className="space-y-1 text-gray-300">
            <div>Is Critical Loading: {criticalLoading.isCriticalLoading ? "✅" : "❌"}</div>
            <div>Is Mutation Loading: {criticalLoading.isMutationLoading ? "✅" : "❌"}</div>
          </div>
        </div>
        <div className="space-y-2">
          ...
          <div className="space-y-1 text-gray-300">
            ...
            <div>Query Loading: {habitsLoading.isQueryLoading ? "✅" : "❌"}</div>
            <div>Mutation Loading: {habitsLoading.isMutationLoading ? "✅" : "❌"}</div>
          </div>
        </div>
      </div>
    </div>
...
// Demo Controls
function DemoControls({ triggerError, setTriggerError, slowQuery, setSlowQuery, onClearCache }: any) {
  return (
    <div className="bg-gray-800 rounded-lg p-4">
  ...
  return (
    <div className="space-y-3">
      <div className="flex justify-between items-center">
        <span className="text-sm text-gray-400">Auto-refetches when controls change</span>
        <FormButton onClick={() => refetch()} variant="ghost" className="text-xs px-2 py-1">
          Manual Refetch
        </FormButton>
      </div>
...
function MutationLoadingButtonDemo({ onError, onSuccess }: any) {
  const mutation = useMutation({
    mutationFn: (data: { action: string }) => mockApiCall(2000, Math.random() > 0.7),
        ...
        <div className="min-h-[150px] bg-gray-700/30 rounded-lg p-4">
          {shouldError ? (
            ...
            <div>
              ...
              <p className="text-sm text-gray-300">
                This component is wrapped in a QueryErrorBoundary. 
                Click the button above to simulate an error.
              </p>
            </div>
          )}
        </div>
...
function CriticalLoadingIndicatorDemo() {
  ...
  return (
    <div className="space-y-3">
      <FormButton onClick={() => refetch()}>
        Trigger Critical Load
      </FormButton>
      ...
      <CriticalLoadingIndicator className="min-h-[150px] bg-gray-700/30 rounded-lg">
        <div className="p-4">
          ...
          <p className="text-sm text-gray-300">
            This only shows loading for first-time loads, not background refetches.
          </p>
          {data && (
        ...
        {customLoading ? (
          <div className="flex items-center justify-center h-full">
            <div className="text-center">
              <LoadingSpinner size="md" />
              <div className="mt-2 text-sm text-gray-400">Custom loading state...</div>
            </div>
          </div>
        ) : (
          <div>
            ...
            <p className="text-sm text-gray-300">
              This demonstrates using the loading hooks with custom logic
